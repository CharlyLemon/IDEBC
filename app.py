...
def get_desocupacion():   return _fetch("desocupacion",   lambda: _inegi("444024"))
def get_informalidad():   return _fetch("informalidad",   lambda: _inegi("444079"))
def get_construccion():   return _fetch("construccion",   lambda: _inegi("400729"))
def get_inpc_tj():        return _fetch("inpc_tj",        lambda: _inegi("628229"))
def get_confianza():      return _fetch("confianza",      lambda: _inegi("516844"))
def get_exportaciones():  return _fetch("exportaciones",  lambda: _inegi("158023"))
...
def _banxico(serie):
    ...
    return None if v in ("N/E", "N/A", "") else float(v)

def get_tipo_cambio():  return _fetch("tipo_cambio", lambda: _banxico("SF43718"), max_days=3)
def get_remesas():      return _fetch("remesas",     lambda: _banxico("SE32"),    max_days=95)
...
# ══════════════════════════════════════════════════════════════════════════════
# FUENTE 4 — SE federal IED
# ══════════════════════════════════════════════════════════════════════════════

def _ied_bc():
    r = requests.get("https://datos.gob.mx/busca/api/3/action/datastore_search",
                     params={"resource_id":"ed67f8f9-c47a-4b76-8a14-1cf1bd3c7fbb",
                             "filters":json.dumps({"entidad":"Baja California"}),
                             "sort":"periodo desc","limit":1}, timeout=15)
    r.raise_for_status()
    recs = r.json().get("result",{}).get("records",[])
    return float(recs[0]["ied_mdd"]) if recs else None
...
INDICATORS = [
    ...
    ("informalidad",  get_informalidad,  "Tasa de informalidad BC",         "%",           "Empleo",    "negative", 30.0,  45.0,   0.25),
    ("imss_bc",       get_imss,          "Trabajadores asegurados IMSS BC", "personas",    "Empleo",    "positive", 580e3, 1050e3, 0.35),
    ("construccion",  get_construccion,  "Valor construcción BC",           "miles MXN",   "Actividad", "positive", 1.2e6, 6.5e6,  1.00),
    ("exportaciones", get_exportaciones, "Exportaciones BC",                "millones USD","Comercio",  "positive", 1500,  4500,   0.70),
    ...
    ("inpc_tj",       get_inpc_tj,       "INPC Tijuana",                    "índice",      "Bienestar", "negative", 2.0,   9.5,    0.35),
    ("confianza",     get_confianza,     "Confianza del consumidor BC",     "índice",      "Bienestar", "positive", 75.0,  115.0,  0.40),
