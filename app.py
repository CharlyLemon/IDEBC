...
def _inegi(serie):
    """
    Nueva versión del BIE (dic 2025): el ID ya incluye área geográfica.
    Se usan dos variantes de URL — primero intenta con área 00 (nueva versión),
    si falla intenta con 0700 (compatibilidad).
    """
    for area in ["00", "0700"]:
        try:
            url = (f"https://www.inegi.org.mx/app/api/indicadores/desarrolladores/"
                   f"jsonxml/INDICATOR/{serie}/es/{area}/false/BIE/2.0/{INEGI_TOKEN}?type=json")
...
INDICATORS = [
    ...
    ("ied_bc",        get_ied,           "IED captada en BC",               "millones USD","Inversión", "positive", 200,   6000,   1.00),  # anual; BC récord ~5,373 mdd en 2023
